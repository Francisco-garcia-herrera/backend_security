...
async function getAllPagesByUnitId(unitId) {
    ...
    try {
        ...
        response.data.forEach(page => {
            htmlResponse += "<li class='list-group-item'><div class='d-flex justify-content-between'><a onclick='getPage(" + page.id + ")'>" + page.name + "</a> <span>" + page.type + "</span> <span>" + page.position + "</span> <a onclick='deletePage(" + page.id + ")'><i class='bi bi-x-circle'></i></a></div></li>";
        });
        htmlResponse += "</ul>";
        document.getElementById("pagesList").innerHTML = htmlResponse;
        resetElementList();
        ...
        response.data.elements.forEach(element => {
            ...
            if (element.elementCarouselDatas) {
                ...
                element.elementCarouselDatas.forEach(data => {
                    htmlResponse += "<li class='list-group-item'><div class='d-flex justify-content-between'><div>" + data.type + "</div> <div><div> position: "+data.position+"</div><a onclick='deleteElementCarouselData(" + data.id + ")'><i class='bi bi-x-circle'></i></a></div></div></li>";
                })
                htmlResponse += "</li>";
                htmlResponse += "</ul>";
            } else {
...
async function getAllUnits() {
    ...
    try {
        ...
        response.data.forEach(unit => {
            htmlResponse += "<li class='list-group-item'><div class='d-flex justify-content-between'><a onclick='getAllPagesByUnitId(" + unit.id + ")'>" + unit.name + "</a> <span>position: "+unit.position+"</span> <a onclick='deleteUnit(" + unit.id + ")'><i class='bi bi-x-circle'></i></a></div></li>";
        });
        htmlResponse += "</ul>";
...
async function downloadZip() {
    ...
    try {
        const response = await axios.get(`${BASE_URL}/units/zip`);
